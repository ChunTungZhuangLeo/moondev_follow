...
MAX_DAYS_TO_RESOLUTION = 9999  # No max - include ALL markets
...
MIN_EDGE_PCT = 0.08  # 8% minimum edge (lowered to allow more trades)
...
ALLOW_POSITION_INCREASE = True  # Allow adding to existing positions if edge improves
...
class KalshiValueAgent:
    ...
    def _print_banner(self):
        ...
        cprint(f"  Min Edge: {MIN_EDGE_PCT*100:.0f}%", "white")
        ...
        cprint(f"  Sweet Spot: {SWEET_SPOT_MIN}-{SWEET_SPOT_MAX} days", "green")
        cprint("="*70 + "\n", "cyan")
    ...
    def _execute_single_trade(self, trade: Dict, market: Dict) -> bool:
        ...
        if analysis.recommended:
            ...
            if signal:
                ...
                contracts = int(position_usd / (market.get('yes_price', 0.5) if side == 'YES' else (1.0 - market.get('yes_price', 0.5))))
                cprint(f"    📈 EXECUTING: {ticker} - {side} | ${position_usd:.2f} ({contracts} contracts) | {edge_pct*100:.1f}% edge", "green", attrs=['bold'])
        ...
        for i, (_, market) in enumerate(df.iterrows()):
            ...
            if result:
                ...
                cprint(f"  ✅ {result['ticker']}: {result['side']} "
                       ...
                       f"{result['edge_pct']*100:.1f}% edge)", "green")
