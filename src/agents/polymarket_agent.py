...
# 🌙 Moon Dev - Profit Margin Filters for High-Value Trades
MIN_PROFIT_MARGIN = 0.05  # Minimum 5% profit margin (e.g., buy YES at 0.40 = 60% potential profit)
MAX_PRICE_FOR_YES = 0.80  # Don't buy YES above this price (max 20% profit margin)
MIN_PRICE_FOR_YES = 0.10  # Don't buy YES below this price (too risky, might resolve NO)
MIN_CONSENSUS_FOR_HIGH_VALUE = 5  # Minimum X out of 6 models must agree

# 🌙 Moon Dev - Position Sizing
BANKROLL_USD = 1000  # Your total trading bankroll in USD
RISK_PER_TRADE_PCT = 0.02  # Risk 2% of bankroll per trade (conservative)

# 🌙 Moon Dev - Filter out sports betting terminology (moneyline, spreads, totals)
IGNORE_BETTING_KEYWORDS = [
    'moneyline', 'spread', 'spreads', 'total', 'totals', 'over/under',
    'point spread', 'handicap', 'asian handicap', 'draw no bet',
    'vs.', 'vs ', ' v ', ' v. ',  # Common sports match formats
]
...
# AI Configuration
USE_SWARM_MODE = True  # Use swarm AI (multiple models) for consensus
AI_MODEL_PROVIDER = "deepseek"  # Model to use if USE_SWARM_MODE = False
AI_MODEL_NAME = "deepseek-chat"  # Model name - DeepSeek is cheap and works!
...
HIGH_VALUE_PICKS_CSV = os.path.join(DATA_FOLDER, "high_value_picks.csv")  # 🌙 Moon Dev - Filtered high-margin trades
...
Provide predictions for each market in the specified format."""
    ...
    def _get_top_consensus_picks(self, swarm_result, markets_df):
        ...
        try:
            ...
            # 🌙 Moon Dev - Save HIGH VALUE picks (filtered by profit margin) to separate CSV
    ...
    def _calculate_profit_margin(self, price, outcome, side):
        """
        🌙 Moon Dev - Calculate potential profit margin for a trade

        Args:
            price: Current market price (0-1) - price of the OUTCOME from API
            outcome: 'Yes' or 'No' - what outcome the API price represents
            side: 'YES' or 'NO' - the side AI recommends we bet on

        Returns:
            tuple: (profit_margin_pct, buy_price, is_high_value)
            - profit_margin_pct: Potential profit if bet wins (e.g., 0.22 = 22%)
            - buy_price: The price you actually pay for the recommended side
            - is_high_value: True if meets all criteria for high-value trade

        Example:
            - If outcome='No', price=0.78 → NO costs $0.78, YES costs ~$0.22
            - If AI recommends side='NO', buy_price=$0.78, profit_margin=22%
            - If AI recommends side='YES', buy_price=$0.22, profit_margin=78%
        """
        try:
            price = float(price)
            outcome = outcome.upper() if outcome else 'YES'
            side = side.upper() if side else 'NO_TRADE'

            # First, determine YES and NO prices from the API data
            if outcome == 'YES':
                yes_price = price
                no_price = 1.0 - price
            elif outcome == 'NO':
            ...
            # Now calculate based on what side AI recommends
            if side == 'YES':
                buy_price = yes_price
                # Profit = $1.00 - cost per share
                profit_margin = 1.0 - yes_price
                # Check if in valid range for YES bets
                is_valid_price = MIN_PRICE_FOR_YES <= yes_price <= MAX_PRICE_FOR_YES
            elif side == 'NO':
                buy_price = no_price
                # Profit = $1.00 - cost per share
                profit_margin = 1.0 - no_price
                # Check if in valid range for NO bets (treat same as YES - good margin range)
                is_valid_price = MIN_PRICE_FOR_YES <= no_price <= MAX_PRICE_FOR_YES
            else:
                # NO_TRADE or invalid
                return (0.0, 0.0, False)

            # Check if meets minimum profit margin
            is_high_value = profit_margin >= MIN_PROFIT_MARGIN and is_valid_price

            return (profit_margin, buy_price, is_high_value)

        except Exception as e:
            cprint(f"⚠️ Error calculating profit margin: {e}", "yellow")
            return (0.0, 0.0, False)

    def _save_high_value_picks_to_csv(self, consensus_response, markets_df):
        """
        🌙 Moon Dev - Save HIGH VALUE picks to dedicated CSV (filtered by profit margin)

        This CSV only contains picks that:
        1. Have strong consensus (>=5/6 models agree)
        2. Have good profit margin (>=5%)
        3. Price is in valid range (not too close to 0 or 1)

        Args:
            consensus_response: The consensus AI's response text
            markets_df: DataFrame of markets being analyzed (contains price data)
        """
        try:
            import re
            from datetime import datetime
            ...
            # Calculate suggested position size based on bankroll
            suggested_bet_usd = BANKROLL_USD * RISK_PER_TRADE_PCT
            cprint(f"💰 Suggested bet size: ${suggested_bet_usd:.2f} (based on ${BANKROLL_USD} bankroll @ {RISK_PER_TRADE_PCT*100:.0f}% risk)", "cyan")
            ...
            for line in lines:
                ...
                # Look for market number and title
                market_match = re.match(r'(\d+)\.\s+Market\s+(\d+):\s+(.+)', line)
                if market_match:
                    if current_pick:
                        picks.append(current_pick)
            ...
            # Convert markets_df to list for price lookup
            markets_list = list(markets_df.iterrows())
            ...
            for pick in picks:
                ...
                side = pick.get('side', '').upper().replace(' ', '_')
                consensus_count = pick.get('consensus_count', 0)

                # Skip if not enough consensus
                if consensus_count < MIN_CONSENSUS_FOR_HIGH_VALUE:
                    cprint(f"   ❌ Market {market_num}: Skipped - Only {consensus_count} models agreed (need {MIN_CONSENSUS_FOR_HIGH_VALUE})", "red")
                    continue

                # Get market title for filtering
                market_title = pick.get('market_title', '').lower()

                # Skip sports betting markets (moneyline, spreads, totals, vs matches)
                is_sports_bet = False
                for keyword in IGNORE_BETTING_KEYWORDS:
                    if keyword.lower() in market_title:
                        is_sports_bet = True
                        cprint(f"   ⚽ Market {market_num}: Skipped - Sports betting market ('{keyword}')", "yellow")
                        break
                if is_sports_bet:
                    continue

                # Get price and outcome from markets_df (market_num is 1-indexed)
                price = 0.5  # Default if not found
                outcome = 'Yes'  # Default if not found
                if 1 <= market_num <= len(markets_list):
                    idx, row = markets_list[market_num - 1]
                    price = float(row.get('price', 0.5))
                    outcome = str(row.get('outcome', 'Yes'))  # Get the outcome (YES or NO)

                # Calculate profit margin using outcome-based pricing
                # price = price of the OUTCOME from API, not always YES
                profit_margin, buy_price, is_high_value = self._calculate_profit_margin(price, outcome, side)

                if is_high_value:
                    pick['outcome_from_api'] = outcome  # Track what the API showed
                    pick['api_price'] = price  # Original price from API
                    pick['buy_price'] = round(buy_price, 2)  # Price you actually pay
                    pick['profit_margin_pct'] = round(profit_margin * 100, 1)
                    high_value_picks.append(pick)
                    cprint(f"   ✅ Market {market_num}: HIGH VALUE! Buy {side} @ ${buy_price:.2f} = {profit_margin*100:.1f}% potential profit", "green", attrs=['bold'])
                else:
                    cprint(f"   ❌ Market {market_num}: {side} @ ${buy_price:.2f} = {profit_margin*100:.1f}% profit (too low or bad price range)", "red")

            if not high_value_picks:
                cprint("\n💔 No HIGH VALUE picks found this round (all margins too low)", "yellow")
                return
            ...
            for pick in high_value_picks:
                buy_price = pick.get('buy_price', 0.5)
                api_price = pick.get('api_price', 0.5)
                outcome_from_api = pick.get('outcome_from_api', 'Yes').upper()
                side = pick.get('side', '')

                # Calculate YES and NO prices for clarity
                if outcome_from_api == 'YES':
                    yes_price = api_price
                    no_price = 1.0 - api_price
                else:
                    no_price = api_price
                    yes_price = 1.0 - api_price

                # Calculate shares you get for your bet
                shares_for_bet = suggested_bet_usd / buy_price if buy_price > 0 else 0
                payout_if_win = shares_for_bet * 1.0  # Each share pays $1 if correct
                potential_profit = payout_if_win - suggested_bet_usd
                ...
                record = {
                    ...
                    'market_title': pick.get('market_title', ''),
                    'side': side,
                    'buy_price': round(buy_price, 2),  # Price you pay for your side
                    'yes_price': round(yes_price, 2),  # Current YES price
                    'no_price': round(no_price, 2),  # Current NO price
                    'profit_margin_pct': pick.get('profit_margin_pct', ''),
                    'suggested_bet_usd': suggested_bet_usd,  # How much to bet
                    'shares_you_get': round(shares_for_bet, 1),  # Number of shares
                    'payout_if_win_usd': round(payout_if_win, 2),  # Total payout if correct
                    'potential_profit_usd': round(potential_profit, 2),  # Profit = payout - cost
                    'consensus_count': pick.get('consensus_count', ''),
                    'total_models': pick.get('total_models', ''),
                    'reasoning': pick.get('reasoning', ''),
                    'link': pick.get('link', '')
            ...
            # Load or create high value picks CSV
            if os.path.exists(HIGH_VALUE_PICKS_CSV):
                hv_df = pd.read_csv(HIGH_VALUE_PICKS_CSV)
            else:
                hv_df = pd.DataFrame(columns=[
                    'timestamp', 'run_id', 'market_title',
                    'side', 'buy_price', 'yes_price', 'no_price', 'profit_margin_pct',
                    'suggested_bet_usd', 'shares_you_get', 'payout_if_win_usd', 'potential_profit_usd',
                    'consensus_count', 'total_models', 'reasoning', 'link'
                ])
            ...
            # Save to CSV
            with self.csv_lock:
                hv_df.to_csv(HIGH_VALUE_PICKS_CSV, index=False)

            cprint(f"\n💎 Saved {len(records)} HIGH VALUE picks to CSV!", "green", attrs=['bold'])
            cprint(f"📁 High Value CSV: {HIGH_VALUE_PICKS_CSV}", "cyan", attrs=['bold'])
            cprint(f"📊 Total high value picks in history: {len(hv_df)}", "cyan")

            # Display summary
            cprint("\n" + "="*80, "green")
            cprint("💎 HIGH VALUE TRADES SUMMARY 💎", "white", "on_green", attrs=['bold'])
            cprint("="*80, "green")
            for pick in high_value_picks:
                ...
                cprint(f"      💲 YES: ${yes_price:.2f} | NO: ${no_price:.2f}", "magenta")
                cprint(f"      💵 Bet ${suggested_bet_usd:.2f} → Get {shares:.1f} shares → Payout ${payout:.2f} = Profit ${profit:.2f} ({pick['profit_margin_pct']}% ROI)", "yellow", attrs=['bold'])
                cprint(f"      ✅ Consensus: {pick['consensus_count']}/{pick['total_models']} AI models agreed", "green")
                cprint(f"      🔗 {pick['link']}", "cyan")
                cprint("", "white")
            cprint("="*80 + "\n", "green")
...
def main():
    ...
    cprint(f"   Consensus Picks: {CONSENSUS_PICKS_CSV}", "white")
    cprint(f"   💎 High Value Picks: {HIGH_VALUE_PICKS_CSV}", "yellow", attrs=['bold'])
    cprint("", "white")
    cprint("💎 High Value Filters:", "yellow", attrs=['bold'])
    cprint(f"   Min Profit Margin: {MIN_PROFIT_MARGIN*100:.0f}%", "white")
    cprint(f"   Min Consensus: {MIN_CONSENSUS_FOR_HIGH_VALUE} models must agree", "white")
    cprint(f"   YES Price Range: {MIN_PRICE_FOR_YES:.2f} - {MAX_PRICE_FOR_YES:.2f}", "white")
    cprint("="*80 + "\n", "cyan")
